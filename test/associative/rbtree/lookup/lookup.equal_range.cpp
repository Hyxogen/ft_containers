...
int main() {
		...
		const ft::pair<rbtree::iterator, rbtree::iterator> range = tree.equal_range(42);
		...
		const ft::pair<rbtree::const_iterator, rbtree::const_iterator> range = tree.equal_range(42);
		assert(range.first == tree.end());
		assert(range.second == tree.end());
        }
}
