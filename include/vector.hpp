...
class vector : public vector_base<Allocator> {
  ...
  protected:
    vector(const vector &other, size_type capacity)
        ...
        if (capacity < other.size()) {
            std::uninitialized_copy(other.begin(), other.begin() + capacity,
                                    begin());
        } else {
            std::uninitialized_copy(other.begin(), other.end(), begin());
        }
  ...
  public:
    ...
    iterator erase(iterator first, iterator last) {
        const iterator it(std::copy(last, end(), first));
        destroy(it, end());
  ...
  protected:
    ...
    void insert_range_aux(iterator pos, ForwardIt first, ForwardIt last,
        ...
        pos = begin() + offset;
        const size_type elements_after = static_cast<size_type>(end() - pos);
        ...
        if (elements_after > count) {
            ...
            std::uninitialized_fill_n(end(), difference, value_type());
            _size += difference;
